...
**按分类抽取**（可选）：
- `/blindbox <分类名称>` - 按盲盒清单里的类别名称抽任务，例如 `/blindbox 德育`、`/blindbox 智育`
...
如果盲盒任务超过了 7 天截止时间但你实际已经完成了，可以通过补交命令提交成果：

```
/blindbox gsubmit 这是我们的任务完成证明...
```

**和普通提交的区别**：

| | /blindbox submit | /blindbox gsubmit |
|---|---|---|
| 何时用 | 7 天截止内 | 7 天截止后 |
| 审核方式 | AI 自动审核 | 管理员手动审核 |
| 积分 | 按任务原始分 | 原始分 - 1 |
| 前提 | 当前有未过期盲盒任务 | 当前有过期盲盒任务 |

**注意事项**：
- ⚠️ gsubmit 只能补交**最近一次过期的盲盒任务**
- ⚠️ 如果过期后又抽取了新盲盒，旧过期任务记录会被覆盖，无法补交
- ⚠️ 补交后不会 AI 自动审核，需要等待管理员处理
- ⚠️ 积分会在原任务分基础上扣 1 分（10 分任务补交可得 9 分）
...
3. **管理员确认** →
   - 审阅 AI 的审核意见
   - 如果同意，执行：`/blindbox pass <提交编号>`
   - 如果不同意，执行：`/blindbox deny <提交编号>`
4. **自动加分** → 确认通过的提交自动添加积分到小组
...
**查看待确认的审核**：
- AI 审核完成后，会在群里发送审核意见和提交编号
- 提交编号格式类似：`abcd1234`

**确认通过**：
```
/blindbox pass abcd1234
```

**确认拒绝**：
```
/blindbox deny abcd1234
```

系统会显示确认结果，包括小组名称、审核意见和应发积分。

### 使用审核管理页面（推荐）

除了群里命令，管理员还可以使用**审核管理页面**来统一处理所有待确认的审核。

**访问审核管理页面**：
```
http://172.26.41.148:6099/astrbot_plugin_blindbox/pages/review/index.html
```

**功能**：
- 📋 显示所有 AI 审核完成但等待管理员确认的提交
- 📊 显示待审核数量和涉及小组数
- 🖼️ 可以查看：
  - 提交人 QQ
  - 提交的文字内容
...
**Q: 如何查看审核结果？**
A: 可以在审核管理页面查看 AI 的审核意见，或者让管理员告诉你。
