...
**批次限制**：每小组每周最多可以抽取 3 次盲盒任务（同一批次）。抽满 3 次后需要等到下一周才能继续抽取。每次抽取时系统会显示 `(本批次第 X/3 次抽取)` 提示。

**任务不重复**：同一小组的每次抽取都会保证与上一次的任务不同，避免连续抽到相同的任务。
...
1. **小组配置与抽取状态**：保存在 AstrBot 的 KV 状态里，包括 `groups`、`member_to_group` 和 `draws`
2. **小组提交/打卡记录**：按小组单独保存为 JSON 文件，路径是 `runtime/submissions/group_<序号>.json`
